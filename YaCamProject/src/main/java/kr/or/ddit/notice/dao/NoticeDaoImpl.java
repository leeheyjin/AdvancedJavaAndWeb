...
public class NoticeDaoImpl implements INoticeDao {
	...
	public List<NoticeVO> listPerPage(Map<String, Object> map) {
		...
		// 2. list = mapper실행
		try {
			list = sqlSession.selectList("Notice.listPerPage", map);
		} finally {
			sqlSession.commit();
			sqlSession.close();
		}
	...
	public int totalCount(Map<String, Object> map) {
		...
		// 2. list = mapper실행
		try {
			count = sqlSession.selectOne("Notice.totalCount", map);
		} finally {
			sqlSession.commit();
			sqlSession.close();
		}
...
//			vo.setComm_date(new java.util.Date()); // 현재 날짜 및 시간 설정
			res = sql.insert("Notice.insertNotice", vo);
		} finally {
			sql.commit();
			sql.close();
		}
	...
	public NoticeVO selectNo(int num) {
		...
		try {
			vo = sqlSession.selectOne("Notice.selectNo", num);
		} finally {
			sqlSession.commit();
			sqlSession.close();
		}
	...
	public int deleteNotice(int num) {
		...
		SqlSession sql = MyBatisSqlSessionFactory.getSqlSession();
		try {
			res = sql.delete("Notice.deleteNotice", num);
		} finally {
			sql.commit();
			sql.close();
		}
	...
	public int updateHit(int num) {
		...
		SqlSession sql = MyBatisSqlSessionFactory.getSqlSession();
		try {
			res = sql.update("Notice.updateHit", num);
		} finally {
			sql.commit();
			sql.close();
		}
