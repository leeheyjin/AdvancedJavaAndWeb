...
<body>
<%
	CommunityVO vo = (CommunityVO)request.getAttribute("commVo");
%>
 <div class="container mt-3">
    <img id="img2" src="./imges/whiteboard.png" style="width: 50px;">
    	<br><br>
   ...
   <h3 style="text-align: center;">게시글 수정</h3>
		<hr>
   <form method="post" enctype="multipart/form-data" action="<%=request.getContextPath()%>/boardUpdate.do">
     <input type="hidden" name="comm_no" value="<%=vo.getComm_no()%>">
     <div class="mb-3 mt-2">
       <label for="sel1" class="form-label">게시글 유형</label>
       <select class="form-select" id="sel1" name="type">
         <option>자유게시판</option>
         <option>후기게시판</option>
         <option>환경보호챌린지</option>
       </select>
     </div>
     <div class="mb-3">
       <label for="subject">제목</label>
       <input type="text" class="form-control" id="subject" placeholder="제목을 입력해주세요" name="ttl" value="<%=vo.getComm_ttl()%>">
     </div>
       <div class="mb-3 mt-3">
         <label for="comment">내용</label>
         <textarea class="form-control" rows="5" id="content" name="dtl" placeholder="내용을 입력해주세요"><%=vo.getComm_dtl()%></textarea>
       </div>
     <div class="mb-3">
       <label for="file">첨부파일</label>
       <input type="file" class="form-control" id="file" name="file">
     </div>
       <button id="send" type="submit" class="btn btn-primary">등록</button>
