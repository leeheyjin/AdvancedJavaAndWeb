...
<body>
<%
	NoticeVO vo = (NoticeVO)request.getAttribute("NotiVo");
%>
        ...
        <a class="backbtn" href="<%=request.getContextPath() %>/LHH/notice/notice.jsp">
            ...
            <h3 style="text-align: center;"><img src="./imges/promotion.png" style="width: 50px;">&nbsp;공지사항 수정</h3>
            <form method="post" enctype="multipart/form-data" action="<%=request.getContextPath()%>/noticeUpdate.do">
              <input type="hidden" name="noti_no" value="<%=vo.getNoti_no()%>">
              <div class="mb-3">
                <label for="subject">제목</label>
                <input type="text" class="form-control" id="subject" placeholder="제목을 입력해주세요" name="ttl" value="<%=vo.getNoti_ttl()%>">
              </div>
                <div class="mb-3 mt-3">
                  <label for="comment">내용</label>
                  <textarea class="form-control" rows="5" id="content" name="dtl" placeholder="내용을 입력해주세요"><%=vo.getNoti_dtl()%></textarea>
                </div>
              <div class="mb-3">
                ...
                <button type="reset" class="btn btn-secondary" onclick="location.href='<%=request.getContextPath()%>/noticeInfo.do?num=<%=vo.getNoti_no()%>'">취소</button>
              </form>
            </div>
